    ...
    <div v-if="mostrarModal" class="modal-overlay" @click.self="fecharModal">
      <div class="modal-content">
        ...
        <div class="filtro-classe">
          <button
            v-for="classe in classesDisponiveis"
            :key="classe"
            :class="['btn-classe', { active: classeAtiva === classe }]"
            @click="classeAtiva = classe"
          >
            {{ classe }}
          </button>
        </div>

        <div v-if="carregandoAssentos" class="loading">Carregando assentos...</div>
        <div v-else-if="erroCarregamento" class="sem-assentos">
          {{ erroCarregamento }}
        </div>
        <div v-else-if="assentosFiltrados.length === 0" class="sem-assentos">
          Nenhum assento encontrado para esta passagem nesta classe.
        </div>
        <div v-else class="lista-opcoes">
          <label
            v-for="assento in assentosFiltrados"
            :key="assento.id"
            :class="['opcao-item', { indisponivel: !assento.available, selecionado: assentoSelecionado?.id === assento.id }]"
          >
            <input
              type="radio"
              name="assento"
              :value="assento.id"
              :checked="assentoSelecionado?.id === assento.id"
              :disabled="!assento.available"
              @change="selecionarAssento(assento)"
            />
            <div class="opcao-conteudo">
              <div class="opcao-topo">
                <strong>{{ assento.seat_number }}</strong>
              </div>
              <div class="detalhes-grid">
                ...
                <div class="detalhe-box">
                  ...
                  <span>{{ assento.trip_id }}</span>
                </div>
                <div class="detalhe-box">
                  <span class="detalhe-box-label">Número Assento</span>
                  <span>{{ assento.seat_number }}</span>
                </div>
                <div class="detalhe-box">
                  <span class="detalhe-box-label">Classe</span>
                  <span>{{ assento.class }}</span>
                </div>
              </div>
            </div>
          </label>
...
function irCarrinho() {
  router.push('/carrinho?from=aereas')
}
...
const assentosDisponiveis = ref([])
const carregandoAssentos = ref(false)
...
const classesDisponiveis = computed(() => {
  const assentosDisponiveisAtivos = assentosDisponiveis.value.filter(a => a.available)
  const classes = [...new Set(assentosDisponiveisAtivos.map(a => a.class).filter(Boolean))]
  return classes
})

// Filtrar assentos por classe - apenas disponíveis
const assentosFiltrados = computed(() => {
  return assentosDisponiveis.value.filter(a => a.class === classeAtiva.value && a.available)
})

// Abrir modal e buscar assentos
async function adicionarAoCarrinho(passagem) {
  ...
  carregandoAssentos.value = true
  erroCarregamento.value = ''
  assentoSelecionado.value = null
  assentosDisponiveis.value = []
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData?.error || 'Erro ao carregar assentos')
    }

    const data = await response.json()
    console.log('Assentos recebidos:', data)
    assentosDisponiveis.value = Array.isArray(data) ? data : []

    if (assentosDisponiveis.value.length > 0) {
      classeAtiva.value = assentosDisponiveis.value[0].class || 'Econômica'
    } else {
      console.warn('Nenhum assento disponível')
    }
  } catch (error) {
    console.error('Erro ao buscar assentos:', error)
    erroCarregamento.value = error.message || 'Erro ao carregar assentos'
    mostrarToast('Erro ao carregar assentos', 'error')
  } finally {
    carregandoAssentos.value = false
    console.log('Modal aberto:', mostrarModal.value)
  }
}

// Selecionar assento
function selecionarAssento(assento) {
  if (!assento.available) return
...
async function confirmarSelecao() {
  if (!assentoSelecionado.value || !passagemSelecionada.value) return

  try {
    console.log('Adicionando ao carrinho:', {
      trip_id: passagemSelecionada.value.id,
      seat_id: assentoSelecionado.value.id,
      cabin_id: null,
      quantity: 1
    })
    ...
    // Isso evita problemas de estado enquanto o modal ainda está aberto
    setTimeout(() => {
    ...
    const errorMsg = error.response?.data?.error || error.message || 'Erro ao adicionar ao carrinho'
    mostrarToast(`Erro: ${errorMsg}`, 'error')
  }
}
